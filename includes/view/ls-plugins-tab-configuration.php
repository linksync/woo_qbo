    ...
    <form  method="POST" name="f1" action="">
        ...
        <center>
            <div>
                ...
                <input type="text" size="30" name="apikey"  value="<?php echo linksync::get_current_laid('No api key'); ?>">
...
<div class="wrap">
    ...
    <fieldset>
        ...
        <div>
            <form method='POST'>
                <input type='submit' style="float: right;margin-top: 10px;" class="button button-primary" title=' Use this button to reset Product and Order Syncing Setting.'   name='rest' value='Reset Syncing Setting'>
            </form>
        </div>
				...
				if( 'disabled' != $options['sync_type'] ){
					...
					if(!$qbo_allow_discount || !$qbo_allow_shipping ){
						...
						if( count($duplicate_products) > 0){
							LS_QBO()->show_woo_duplicate_products( $duplicate_products );
							$show_manual_sync = false;
						}
					}
