...
class LS_Duplicate_Sku_List extends WP_List_Table
{
    ...
    public function column_product_name($item)
    {
        ...
        $delete_link = LS_QBO_Menu::admin_url(LS_QBO_Menu::linksync_page_menu_url('duplicate_sku&action=delete_permanently&ID=' . $item['ID']));
        //Build row actions
        $actions = array(
    ...
    public function process_bulk_action()
    {
        ...
        if ('delete_permanently' === $this->current_action()) {
            if (!empty($_REQUEST['ID'])) {
                ...
                $url = LS_QBO_Menu::linksync_page_menu_url('duplicate_sku');
                wp_redirect($url);
                exit();
            }
        }
