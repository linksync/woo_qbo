...
class LS_QBO_Order_Form{
	...
	public function __construct() {
		...
		if( is_null( $this->order_options ) ){
			$this->order_options = LS_QBO::order_option();
	...
	public function sync_type(){
		...
		<fieldset id="qbo-order-configuration">
			...
			<p>
				...
				<input name="order_sync_type" type="radio" id="ls-qbo-disabled" <?php echo ($option == $this->sync_types[1] ) ? 'checked' : ''; ?> value="<?php echo $this->sync_types[1]; ?>" >
		...
		<tr valign="top">
			...
			<td class="forminp forminp-checkbox">
					...
					<div id="sales_receipt_container" <?php echo ($option['post_quikbooks_as']['receipt_type'] != $this->receipt_types[0]) ? 'style="display:none;"': ''; ?>>
							...
							<?php
								if( !empty($option['asset_accounts']) ){
									...
									foreach($option['asset_accounts'] as $assets_account){
										...
										?>
										<option <?php echo $selected; ?> value="<?php echo $assets_account['fullyQualifiedName']; ?>">
						...
						<div class="sub-option">
							...
							<div class="sub-option">
								...
								<div class="" id="mapped_payment_method_container" <?php echo ($option['post_quikbooks_as']['payment_method'] != $this->payment_methods[1]) ? 'style="display:none;"' : ''; ?>>
									<table>
										<thead>
										<tr>
											<th>Woo-Commerce</th><th>QuickBooks Online</th>
										</tr>
										</thead>
										<tbody>
										<?php
				...
				<div >
					...
					<div id="sales_invoice_container" class="sub-option" <?php echo ($option['post_quikbooks_as']['receipt_type'] != $this->receipt_types[1]) ? 'style="display:none;"': ''; ?>>
				...
				<label>
					<?php
						if(empty($option['location_list'])){
							...
							?>
							<input name="location_checkbox" type="checkbox" <?php echo ($option['location']['location_status']=='on') ? 'checked': ''; ?> >
							Enable
							<?php
							help_link(array(
				...
				<label>
					<?php
						if( empty($qbo_classes) ){
							...
							?>
							<input name="class_checkbox" type="checkbox" <?php echo ($option['class_status']=='on') ? 'checked': ''; ?> >
							Enable
							<?php
							help_link(array(
								'title' => 'Select this option if you\'d like customer data, such as name, email address and shipping and billing address, to be included when exporting orders to Vend.'
